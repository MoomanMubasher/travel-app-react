...
export default function Ratings(props) {
    ...
    const deleteProduct = async(data) => {
        ...
        try {
            ...
            const response = await axios.delete(`http://localhost:8080/api/landmarks/${id}/ratings/${product._id}`,config);
            console.log(response,'response')
            setProducts(_products);
            setDeleteProductDialog(false);
            setProduct(emptyProduct);
            toast.current.show({ severity: 'success', summary: 'Successful', detail: 'Rating Deleted', life: 3000 });
          } catch (error) {
            toast.current.show({ severity: 'error', summary: 'Error', detail: 'Error deleting rating. Please try again.', life: 3000 });
          }
    ...
    const header = (
        <div className="flex flex-wrap gap-2 align-items-center justify-content-between">
            <h4 className="m-0">Manage Ratings</h4>
    ...
    const saveProduct = async () => {
        ...
        if (product.rating) {
            ...
            try {
                ...
                await axios.put(`http://localhost:8080/api/landmarks/${id}/ratings/${_product?._id}`, {
                ...
                toast.current.show({ severity: 'error', summary: 'Error', detail: 'Error rating comment. Please try again.', life: 3000 });
              }
    ...
    return (
        ...
        <div>
            ...
            <Dialog visible={productDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Rating" modal className="p-fluid" footer={productDialogFooter} onHide={hideDialog}>
                <div className="field">
                    <label htmlFor="rating" className="font-bold">
                        Change Rating
                    </label>
                    <InputNumber id="rating" value={product.rating} onValueChange={(e) => onInputChange(e, 'rating')} />
                </div>
